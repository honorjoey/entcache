...
type (
	...
	Options struct {
		...
		// Prefix is the prefix for cache keys.
...
// Prefix configures the prefix for cache keys.
...
func (d *Driver) Query(ctx context.Context, query string, args, v any) error {
	...
	if (!strings.HasPrefix(query, "SELECT") && !strings.HasPrefix(query, "select")) || strings.Contains(query, "@@") {
...
func (d *Driver) optionsFromContext(ctx context.Context, query string, args []any) (ctxOptions, error) {
	...
	if cq := ent.QueryFromContext(ctx); cq != nil {
		table = cq.Type
	}
	if opts.key == nil {
		...
		opts.key = fmt.Sprintf("%s:%s:%v", d.Prefix, table, key)
	}
	if opts.ttl == 0 {
		opts.ttl = d.TTL
	}
